...
def test_compare():
    ...
    assert a < b
    assert b > a
    c = QuantifiedValue(name="apple", quantity=16)
    assert a < c
    assert b < c
    assert c >= b


def test_collect_field_aliases():
    from ipa.pydantic import collect_field_aliases

    class MyModel(BaseModel):
        name: str = Field(alias="名称")
        age: int = Field(alias="年龄")

    f = MyModel.model_fields["name"]
    assert collect_field_aliases(f) == {"名称"}

    f = MyModel.model_fields["age"]
    assert collect_field_aliases(f) == {"年龄"}
